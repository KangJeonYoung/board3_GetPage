		...
		<div class ="panel panel-default">
			...
			<div class="panel-body">
				...
				<c:if test="${user == board.writer  ||  model == '1'}">
					<button type="modify" class="btn btn-default" onclick="location.href='/board/tip_modify?bno=<c:out value="${board.bno }"/>'">글 수정</button>
				</c:if>
				<button type="list" class="btn btn-default" onclick="location.href='/board/tip_list'">리스트</button>
			</div>
			<!-- end panel-body -->
		</div>
		<!-- end panel-body -->
...
<!-- /.row -->
	...
	<form role="form" action="/replies/tip_register" method="post">
		<div class = "panel panel-default">
				...
				<ul class = "chat">
					...
					<li class = "left clearfix" data-rno = '12'>
						<div>
							...
							<form action="/replies/tip_remove" method="get">
							<input type="hidden" name="rno" value="${replies.rno }">
							<input type="hidden" name="replyer" value="${replies.replyer}">
							<button type="button" onclick="location.href='/replies/tip_modify?rno=<c:out value="${replies.rno }"/>'" class='btn btn-primary btn-xs pull-right'>수 정</button>
